 ...
 */
enum class KEY_NAMES : std::int8_t
{
 ...
 */
class EventsHandler : public sigc::trackable
{
public:
  using ClickSignal    = sigc::signal<void(std::int32_t, std::int32_t)>;
  using ReleasedSignal = sigc::signal<void(void)>;

  explicit EventsHandler(const std::shared_ptr<sf::RenderWindow>& window);

  /**
   * @brief Polls the main window.
   */
  void poll();
  ...
  ClickSignal signalClicked() const;

  ClickSignal signalRightClick() const;

  ReleasedSignal signalMouseReleased() const;
...
private:
  ...
  ClickSignal                       m_left_click_signal;
  ClickSignal                       m_right_click_signal;
  ReleasedSignal                    m_mouse_released_signal;
};
