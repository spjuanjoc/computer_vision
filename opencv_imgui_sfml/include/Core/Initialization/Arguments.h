...
/**
 * Declares the arguments that the application may receive.
 */
struct Arguments
{
  bool                      is_fullscreen {};
...
static Arguments parseArguments(int argc, const char* argv[])
{
  ...
  parser.add_argument("-f", "--fullscreen").help("The window is fullscreen").default_value(false).implicit_value(true).nargs(0);
  ...
  arguments.is_fullscreen = parser.get<bool>("--fullscreen");
